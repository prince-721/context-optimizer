...
import { ProjectMemory, ExportFormat } from '../types';
import { formatTokenCount } from '../utils/tokenizer';

export class Exporter {
  export(memory: ProjectMemory): ExportFormat {
    return {
  ...
  toOptimizedPrompt(m: ProjectMemory): string {
    const lines: string[] = [];

    lines.push(`# Project: ${m.projectName}`);
    if (m.framework) lines.push(`Framework: ${m.framework} (${m.language})`);
    ...
    const rules: string[] = [];
    if (m.completedFeatures.length > 0) rules.push('Never rewrite completed modules.');
    if (m.folderStructure) rules.push('Maintain existing folder structure.');
    if (m.namingConvention) rules.push(`Use ${m.namingConvention} naming.`);
    if (rules.length > 0) lines.push(`\nRules: ${rules.join(' ')}`);

    return lines.join('\n');
  }

  toMarkdown(m: ProjectMemory): string {
    ...
    if (m.codingStyle.stateManagement) sections.push(`- State: ${m.codingStyle.stateManagement}`);
    if (m.codingStyle.patterns.length) sections.push(`- Patterns: ${m.codingStyle.patterns.join(', ')}`);
    ...
    if (m.developerNotes.length > 0) {
      sections.push(`\n## 📝 Developer Notes`);
      for (const n of m.developerNotes) sections.push(`- ${n}`);
    }
    ...
    sections.push(`|--------|-------|`);
    sections.push(`| Original Tokens | ${formatTokenCount(m.tokenStats.original)} |`);
    sections.push(`| Compressed Tokens | ${formatTokenCount(m.tokenStats.compressed)} |`);
  ...
  toMinJson(m: ProjectMemory): string {
    const mini = {
      p: m.projectName,
      f: m.framework,
      l: m.language,
  ...
  generateReadme(m: ProjectMemory): string {
    ...
    lines.push('\n## Features');
    for (const f of m.completedFeatures) lines.push(`- ✅ ${f.name}`);
    for (const f of m.pendingFeatures) lines.push(`- 🔲 ${f.name} _(pending)_`);
    ...
    if (m.dependencies.runtime['next']) lines.push(`${m.dependencies.packageManager} run dev`);
    else lines.push(`${m.dependencies.packageManager} start`);
    lines.push('```');
  ...
  generateOnboarding(m: ProjectMemory): string {
