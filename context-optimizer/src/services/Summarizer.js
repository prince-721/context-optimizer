...
class Summarizer {
    summarizeFile(filePath, content) {
        const ext = path.extname(filePath).toLowerCase();
        const name = path.basename(filePath).toLowerCase();
        if (name === 'package.json')
            return this.summarizePackageJson(content);
        if (name.endsWith('.env.example') || name === '.env')
            return this.summarizeEnv(content);
        if (name === 'readme.md')
            return this.summarizeReadme(content);
        if (ext === '.prisma')
            return this.summarizePrisma(content);
        if (name.includes('schema') && (ext === '.sql' || ext === '.js' || ext === '.ts'))
            return this.summarizeSchema(content);
        if (name.includes('route') || name.includes('router'))
            return this.summarizeRoutes(content);
        if (ext === '.ts' || ext === '.tsx' || ext === '.js' || ext === '.jsx')
            return this.summarizeCode(content, filePath);
        if (ext === '.json')
            return this.summarizeJson(content);
        if (ext === '.yml' || ext === '.yaml')
            return this.summarizeYaml(content, filePath);
    ...
    summarizeCode(content, filePath) {
        ...
        if (arrows.length)
            parts.push(`Exports: [${arrows.slice(0, 10).join(', ')}]`);
        // JSX components
    ...
    summarizeYaml(content, filePath) {
    ...
    extractDependencies(packageJsonContent) {
        try {
    ...
    detectFramework(deps) {
        ...
        if (d['svelte'])
            return 'Svelte';
        if (d['react'])
            return d['react-native'] ? 'React Native' : 'React';
        if (d['vue'])
            return 'Vue.js';
        if (d['express'])
            return 'Express.js';
    ...
    detectCodingStyle(content) {
        const style = { patterns: [] };
    ...
    detectDatabase(deps) {
        const d = { ...deps.runtime, ...deps.dev };
        if (d['mongoose'] || d['mongodb'])
            return { type: 'MongoDB', schemas: [], orm: 'Mongoose' };
        if (d['prisma'] || d['@prisma/client'])
            return { type: 'Prisma (SQL)', schemas: [], orm: 'Prisma' };
        if (d['sequelize'])
            return { type: 'SQL', schemas: [], orm: 'Sequelize' };
        if (d['pg'] || d['postgres'])
            return { type: 'PostgreSQL', schemas: [], orm: 'pg' };
        if (d['mysql'] || d['mysql2'])
            return { type: 'MySQL', schemas: [], orm: 'mysql2' };
        if (d['sqlite3'] || d['better-sqlite3'])
            return { type: 'SQLite', schemas: [], orm: 'sqlite3' };
        if (d['firebase'] || d['firebase-admin'])
            return { type: 'Firebase Firestore', schemas: [] };
        if (d['@supabase/supabase-js'])
            return { type: 'Supabase (PostgreSQL)', schemas: [] };
        if (d['redis'] || d['ioredis'])
            return { type: 'Redis', schemas: [] };
