...
export class DashboardPanel {
  ...
  static createOrShow(context: vscode.ExtensionContext, memory: ProjectMemory) {
  ...
  private constructor(panel: vscode.WebviewPanel, memory: ProjectMemory) {
    this.panel = panel;
  ...
  update(memory: ProjectMemory) {
    this.panel.webview.html = this.getHtml(memory);
  }

  private getHtml(m: ProjectMemory): string {
    const saved = m.tokenStats.savedPercent;
    const progressColor = saved >= 80 ? '#4ade80' : saved >= 50 ? '#facc15' : '#f87171';

    const completedItems = m.completedFeatures.map(f =>
      `<div class="feature-item done"><span class="badge done">✓</span>${f.name}</div>`
    ).join('');

    const pendingItems = m.pendingFeatures.map(f =>
      `<div class="feature-item pending"><span class="badge pending">○</span>${f.name}</div>`
    ).join('');

    const bugItems = m.knownBugs.map(b =>
      `<div class="bug-item ${b.severity}"><span class="sev">[${b.severity}]</span> ${b.description}</div>`
    ).join('');

    const recentChanges = m.gitHistory.slice(-5).reverse().map(g =>
      `<div class="change-item"><code>${g.hash}</code> ${g.message}</div>`
    ).join('');

    const apiList = m.apiEndpoints.slice(0, 10).map(ep =>
      `<span class="endpoint"><span class="method ${ep.method.toLowerCase()}">${ep.method}</span> ${ep.path}</span>`
    ).join('');
  ...
  <div class="card">
    ...
    <div class="token-ring">
      ...
      <div class="token-stats">
        <div class="token-stat"><span class="tk-label">Original</span><span class="tk-value">${formatTokenCount(m.tokenStats.original)}</span></div>
        <div class="token-stat"><span class="tk-label">Compressed</span><span class="tk-value">${formatTokenCount(m.tokenStats.compressed)}</span></div>
        <div class="token-stat"><span class="tk-label">Eliminated</span><span class="tk-value" style="color:${progressColor}">${formatTokenCount(m.tokenStats.original - m.tokenStats.compressed)}</span></div>
      </div>
    </div>
  </div>
